...
class CaptioningRNN(object):
  ...
  def loss(self, features, captions):
    ...
    ############################################################################
    proj_features, proj_cache = affine_forward(features, W_proj, b_proj)
    embed_captions, embed_cache = word_embedding_forward(captions_in, W_embed)
    if self.cell_type == 'rnn':
      rnn_h, rnn_cache = rnn_forward(embed_captions, proj_features, Wx, Wh, b)
      vocab_scores, vocab_cache = temporal_affine_forward(
          rnn_h, W_vocab, b_vocab)
      loss, dscores = temporal_softmax_loss(vocab_scores, captions_out, mask)
      d_rnn_h, grads['W_vocab'], grads['b_vocab'] = temporal_affine_backward(
          dscores, vocab_cache)
      d_embed_captions, d_proj_features, grads['Wx'], grads['Wh'], grads[
          'b'] = rnn_backward(d_rnn_h, rnn_cache)
    elif self.cell_type == 'lstm':
      lstm_h, lstm_cache = lstm_forward(embed_captions, proj_features, Wx, Wh,
                                        b)
      vocab_scores, vocab_cache = temporal_affine_forward(
          lstm_h, W_vocab, b_vocab)
      loss, dscores = temporal_softmax_loss(vocab_scores, captions_out, mask)
      d_lstm_h, grads['W_vocab'], grads['b_vocab'] = temporal_affine_backward(
          dscores, vocab_cache)
      d_embed_captions, d_proj_features, grads['Wx'], grads['Wh'], grads[
          'b'] = lstm_backward(d_lstm_h, lstm_cache)
    else:
      raise ValueError('Unrecognized cell type: %s' % self.cell_type)

    grads['W_embed'] = word_embedding_backward(d_embed_captions, embed_cache)
    d_features, grads['W_proj'], grads['b_proj'] = affine_backward(
        d_proj_features, proj_cache)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
  ...
  def sample(self, features, max_length=30):
    ...
    as
    the next word. The initial hidden state is computed by applying an
    affine
    ...
    if self.cell_type == 'rnn':
      ...
      for step in xrange(max_length):
        next_h, next_c, _ = lstm_step_forward(embed_curr_token, prev_h, prev_c,
                                              Wx, Wh, b)
        vocab_scores, _ = affine_forward(next_h, W_vocab, b_vocab)
        words = np.argmax(vocab_scores, axis=1)
        words[np.where(words == self._end)] = self._null
